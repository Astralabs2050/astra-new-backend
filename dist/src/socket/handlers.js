...
const test = (socket) => {
    ...
    socket.on("start_chat", async (receiverId) => {
        try {
            ...
            const messages = await model_1.MessageModel.findAll({
                where: {
                    [sequelize_1.Op.or]: [
    ...
    socket.on("send_message", async (data) => {
        ...
        try {
            ...
            const newMessage = await model_1.MessageModel.create({
                senderId: socket.user.id,
                senderName: socket.user.name,
                receiverId: data.receiverId,
                content: data.content,
    ...
    socket.on("mark_as_read", async (messageId) => {
        try {
            const message = await model_1.MessageModel.findByPk(messageId);
