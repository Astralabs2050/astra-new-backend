...
exports.typing = exports.markAsRead = exports.updateUserAvailability = exports.handlePrivateMessage = exports.translateMessage = exports.getPreviousMessages = exports.receiveMessage = exports.sendMessage = void 0;
...
const saveAndBroadcastMessage = async (data) => {
    var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k, _l, _m, _o, _p, _q, _r, _s, _t, _u, _v, _w, _x, _y, _z, _0;
    try {
        // Check if the receiver is online
        const receiver = await model_1.UsersModel.findOne({
            where: { id: data.receiverId },
            attributes: ["active"],
        });
        // Create the message with a seen status based on receiver's availability
        const message = await ChatMessage_model_1.MessageModel.create({
        ...
        const receiverData = await model_1.UsersModel.findOne({
            where: { id: data.receiverId },
            include: [
                {
        ...
        const senderData = await model_1.UsersModel.findOne({
...
async function getPreviousMessages(socket) {
    socket.on("get_previous_messages", async (data) => {
        try {
...
async function updateUserAvailability(status, id) {
    try {
        await model_1.UsersModel.update({ active: status }, { where: { id } });
    }
    catch (error) {
...
function markAsRead(io) {
    io.on("connection", (socket) => {
        socket.on("mark_as_read", async (data) => {
            try {
                await ChatMessage_model_1.MessageModel.update({ seen: true }, {
                    where: {
                        receiverId: data.receiverId,
                        senderId: data.senderId,
                        seen: false,
