...
class DesignClass {
    constructor() {
        ...
        this.generateNewDesign = async (data, userId) => {
            const transaction = await db_1.sequelize.transaction(); // Start a new transaction
            try {
        ...
        `;
                ...
                const generateDesign = async () => {
                    const imageResponse = await axios_1.default.post(imageUrl, requestData, {
                        headers: {
                            Authorization: `Bearer ${apiKey}`,
                            "Content-Type": "application/json",
                        },
                ...
                const userExists = await model_1.UsersModel.findByPk(userId);
                if (!userExists) {
                    return { status: false, message: "User ID not found in the database." };
                }
                // Create a new design in the database
                const newDesign = await design_model_1.DesignModel.create({
                ...
                const mediaEntries = imageUrls.map(async (url) => {
        ...
        this.uploadNewDesign = async (data, userId) => {
            const transaction = await db_1.sequelize.transaction(); // Start a transaction
            try {
                ...
                await media_model_1.MediaModel.bulkCreate(mediaRecords, { transaction });
        ...
        this.addCreatorToDesign = async (designId, creator) => {
            try {
                //check if the design id is valid
                const design = await design_model_1.DesignModel.findOne({
                    where: { id: designId },
                });
                if (!design) {
                    return {
        ...
        this.additionalInformation = async (designId, data) => {
            var _a;
            const transaction = await db_1.sequelize.transaction();
            try {
                // Validate Design
                const design = await design_model_1.DesignModel.findOne({ where: { id: designId } });
                if (!design) {
                    await transaction.rollback();
                    return { message: "No design found", status: false };
                }
                // Update Design
                await design.update({ outfitName: data === null || data === void 0 ? void 0 : data.outfitName, pieceNumber: data === null || data === void 0 ? void 0 : data.pieceNumber }, { transaction });
                // Create Pieces
                const createdPieces = await Promise.all((_a = data === null || data === void 0 ? void 0 : data.pieces) === null || _a === void 0 ? void 0 : _a.map(async (piece) => model_1.PieceModel.create({
                    designId: design.id,
                ...
                // Upload All Images
                const uploadResults = await Promise.all(allUploads.map((upload) => (0, aws_1.uploadImageToS3)(upload.view || "PRINT", upload.image, upload.pieceId)));
