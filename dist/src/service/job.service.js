    ...
    constructor() {
        this.createJob = async (data, userId) => {
            ...
            try {
                ...
                const existingJob = await model_1.JobModel.findOne({
                    where: { designId },
                });
                if (existingJob) {
                    return {
                ...
                const newJob = await model_1.JobModel.create({
                    description,
                    timeline,
                    manufacturer,
                    designId,
        ...
        this.updateJob = async (jobId, timelineStatus) => {
            try {
                const updatedJob = await model_1.JobModel.update({ timelineStatus }, { where: { id: jobId } });
                if (updatedJob[0] === 0) {
                    return {
        ...
        this.getSavedJob = async (userId) => {
            try {
                //get all saved jobs by userId
                const savedJobs = await savedJob_model_1.SavedJobsModel.findAll({
                    where: { userId },
                    include: [
                        {
        ...
        this.acceptDeclineJobApplication = async (jobId, status, negotiation) => {
            const transaction = await db_1.sequelize.transaction();
            try {
                // Fetch the job application
                const jobApplicationFound = await jobApplication_model_1.JobApplicationModel.findByPk(jobId);
                if (!jobApplicationFound) {
                    return { message: "Job application not found", status: false };
                }
                // Update the job application status
                const [updatedRows] = await jobApplication_model_1.JobApplicationModel.update({ status, negotiation }, { where: { id: jobId }, transaction });
                ...
                if (status) {
                    ...
                    const userExists = await model_1.UsersModel.findByPk(jobApplicationFound.userId);
                    if (!userExists) {
                        throw new Error("The associated user for the brandId does not exist. Update failed.");
                    }
                    await model_1.JobModel.update({
        ...
        this.getOngoingJobApplication = async (id, filter) => {
            try {
                const whereClause = {
                    makerId: id,
                };
                ...
                const getJob = await model_1.JobModel.findAll({
                    where: whereClause,
                    include: [
                        {
                            model: model_1.DesignModel,
        ...
        this.saveJob = async (userId, jobId) => {
            try {
                // check if the job is valid
                const job = await model_1.JobModel.findOne({ where: { id: jobId } });
                if (!job) {
                    return {
                ...
                //save job
                const saveJob = await savedJob_model_1.SavedJobsModel.create({
                    jobId,
                    userId,
                });
                return {
        ...
        this.generateJobDescWithAi = async (design) => {
            var _a, _b, _c;
            try {
                // Fetch job details
                if (!design) {
      ...
      `;
        ...
        this.getJob = async (userId, status) => {
            try {
                const jobs = await model_1.JobModel.findAll({
        ...
        this.getEachJob = async (jobId, authUser) => {
            var _a, _b, _c;
            try {
                const job = await model_1.JobModel.findOne({
                    where: { id: jobId },
                    include: [
                        {
                            model: model_1.DesignModel,
                ...
                if (((_a = job === null || job === void 0 ? void 0 : job.dataValues) === null || _a === void 0 ? void 0 : _a.userId) !== (authUser === null || authUser === void 0 ? void 0 : authUser.id)) {
                    ...
                    if (((_c = job === null || job === void 0 ? void 0 : job.dataValues) === null || _c === void 0 ? void 0 : _c.impression) === null) {
                        await model_1.JobModel.update({ impression: 1 }, { where: { id: jobId } });
                    }
                    await model_1.JobModel.increment("impression", { where: { id: jobId } });
                }
                return {
                    status: true,
                    message: "gotten job",
        ...
        this.applyForJob = async (data, userId) => {
            const transaction = await db_1.sequelize.transaction();
            try {
                // Check if the job is valid
                const job = await model_1.JobModel.findOne({
                ...
                const existingApplication = await jobApplication_model_1.JobApplicationModel.findOne({
                ...
                if ((data === null || data === void 0 ? void 0 : data.projectIds) && Array.isArray(data.projectIds)) {
                    ...
                    for (const projectId of data.projectIds) {
                        const project = await model_1.ProjectModel.findOne({
                            where: { id: projectId },
                        });
                        if (!project) {
                            return {
                ...
                const newApplication = await jobApplication_model_1.JobApplicationModel.create({
                    userId,
                ...
                if ((data === null || data === void 0 ? void 0 : data.projectIds) && Array.isArray(data.projectIds)) {
                    ...
                    await model_1.JobApplicationProjects.bulkCreate(projectAssociations, {
                        transaction,
                    });
                }
                await transaction.commit();
        ...
        this.getJobApplicants = async (jobId) => {
            try {
                ...
                const { rows: jobApplications, count: totalApplications } = await jobApplication_model_1.JobApplicationModel.findAndCountAll({
                    where: { jobId },
                    include: [
                        {
                            model: model_1.UsersModel,
        ...
        this.getOneJobApplicants = async (jobId, userId) => {
            var _a, _b, _c, _d, _e;
            try {
                // Check if the job exists
                const job = await model_1.JobModel.findOne({
                ...
                // Get applications for the job with pagination
                const jobApplications = await jobApplication_model_1.JobApplicationModel.findOne({
                    where: {
